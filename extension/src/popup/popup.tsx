...
const Popup: React.FC<{}> = () => {
  ...
  chrome.storage.onChanged.addListener((changes, namespace) => {
    ...
    if (changes.links?.newValue) {
      if (changes.links.newValue.length !== changes.links.oldValue.length) {
        const currentLink = links?.newValue.filter(
          ({ id: id1 }) =>
            !changes?.links?.oldValue.some(({ id: id2 }) => id2 === id1)
        )[0];

        setBookmark(currentLink);
      } else {

        setBookmark(changes.links.newValue.find((b) => b.id == bookmark.id));
      }
    }
  });
  ...
  const getCookie = async () => {
    await chrome.cookies
      ...
      .then((c) => {
        if (c?.value) {
          ...
          chrome.tabs.query(
            ...
            (tabs) => {
              if (tabs.length > 0) {
                const redirectUrl = encodeURIComponent(tabs[0].url);
                chrome.tabs.create({
                  url: `http://testing.mygptbrain.com/onboarding/login?redirect=${redirectUrl}`,
                });
              }
            }
          );
      ...
      .catch((err) => {
        console.log(err, "err");
        if (err?.message == "The cookie is missing a required attribute.") {
          chrome.tabs.create({
    ...
    <ChakraProvider theme={theme}>
      {cookie && (
        <Flex className="popup" direction="column" gap={4} m={6}>
          ...
          <Text fontSize="md" noOfLines={3} textTransform="capitalize">
            {bookmark?.heading}
          </Text>
